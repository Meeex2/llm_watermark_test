...
class Model(nn.Module):
    def __init__(
        ...
        # Initialize DDIM and DPM-Solver
        from tabsyn.ddim_utils import DDIMSampler, DPMSolver

        self.ddim_sampler = DDIMSampler(num_timesteps=1000, device="cuda")
        self.dpm_solver = DPMSolver(num_timesteps=1000, device="cuda")

    def forward(self, x):
        loss = self.loss_fn(self.denoise_fn_D, x)
        return loss.mean(-1).mean()

    def sample_ddim(self, num_samples, dim, num_steps=50, eta=0.0):
        """Generate samples using DDIM."""
        device = next(self.parameters()).device
        x = torch.randn(num_samples, dim, device=device)

        for t in range(num_steps - 1, -1, -1):
            t_tensor = torch.tensor([t], device=device).repeat(num_samples)
            x = self.ddim_sampler.remove_noise(self.denoise_fn_D, x, t_tensor, eta)

        return x

    def recover_noise(self, x, t, num_steps=20):
        """Recover noise from a noisy image using DPM-Solver."""
        return self.dpm_solver.recover_noise(self.denoise_fn_D, x, t, num_steps)
